...
public class ChampionShop : MonoBehaviour
{
    ...
    public Champion GetRandomChampionInfo()
    {
        ...
        {
            //randomise a number
            rand = Random.Range(0, gameData.championsArray.Length);
            while (rand < gameData.championsArray.Length && GlobalGameData.getInstance().chosenChampion[rand] == 0)
            {
                rand++;
            }
            if (rand < gameData.championsArray.Length && GlobalGameData.getInstance().chosenChampion[rand] == 1)
            {
                break;
            }
        }
